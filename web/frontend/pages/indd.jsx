...
export default function QuoteRequestsPage() {
  ...
  const fetchQuoteData = async () => {
    try {
      ...
      let status = statusFilter.length > 0 ? statusFilter.join(",") : undefined;
      let search = searchQuery ? searchQuery : undefined;
      
      // Handle date range based on time filter
      let dateFrom, dateTo;
      ...
      if (response.status && response.result && response.result.list) {
        ...
        setTotalQuotes(response.result.total || response.result.list.length);
      } else {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        if (storeResponse.status) {
          ...
          setError("Failed to fetch store data: " + storeResponse.message);
          setIsLoading(false);
        }
      } catch (error) {
    ...
    fetchAllData();
  }, [currentPage, statusFilter, timeFilter, searchQuery, dateRange]);
  ...
  const handleStatusChange = useCallback((value) => {
    setStatusFilter(value.filter(v => v !== "All Status"));
    setCurrentPage(1); // Reset to first page when filter changes
  }, []);

  const handleTimeChange = useCallback((value) => {
    setTimeFilter(value.filter(v => v !== "All Time"));
    setCurrentPage(1); // Reset to first page when filter changes
  }, []);
  ...
  const handleTimeRemove = useCallback(() => setTimeFilter([]), []);
  const handleSearchQueryRemove = useCallback(() => setSearchQuery(""), []);
  const handleQueryValueRemove = useCallback(() => setSearchQuery(""), []);
  const handleFiltersClearAll = useCallback(() => {
    handleStatusRemove();
    handleTimeRemove();
    handleSearchQueryRemove();
    setDateRange({ from: "", to: "" });
  }, [handleStatusRemove, handleTimeRemove, handleSearchQueryRemove]);
  ...
  if (timeFilter.length > 0) {
    appliedFilters.push({
      key: "timeFilter",
      label: disambiguateLabel("timeFilter", timeFilter),
            ...
            <Card>
              <InlineStack align="space-between" blockAlign="center">
                <BlockStack gap="100">
                  ...
                  <Text variant="bodyLg" fontWeight="semibold">
                    {quoteData.filter((q) => q.quoteStatus === "SUBMITTED").length}
                  </Text>
                </BlockStack>
                <div
                  style={{
            ...
            <Card>
              <InlineStack align="space-between" blockAlign="center">
                <BlockStack gap="100">
                  ...
                  <Text variant="bodyLg" fontWeight="semibold">
                    {quoteData.filter((q) => q.quoteStatus === "ACCEPTED").length}
                  </Text>
                </BlockStack>
                <div
                  style={{
...
//   // Fetch quote data from API with filters
//   const fetchQuoteData = async (search = searchQuery, status = statusFilter, dateFrom, dateTo, page = currentPage) => {
//     try {
//       setIsLoading(true);
//       setError(null);
      
//       // Prepare filters for API
//       let statusString = status.length > 0 && !status.includes("") ? status.join(",") : undefined;
//       let searchString = search ? search : undefined;
...
//         dateFrom = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().split('T')[0];
//         dateTo = new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString().split('T')[0];
//       } else if (timeFilter.includes("All Time")) {
//         dateFrom = "";
//         dateTo = "";
//       } else if (dateRange.from || dateRange.to) {
//         dateFrom = dateRange.from;
...
//     fetchAllData();
//   }, [currentPage, statusFilter, timeFilter, dateRange]);
...
//   const handleStatusChange = useCallback((value) => {
//     // If "All Status" is selected, pass empty array
//     const filteredStatus = value.includes("") ? [] : value.filter(v => v !== "");
//     setStatusFilter(filteredStatus);
//     setCurrentPage(1);
//   }, []);

//   const handleTimeChange = useCallback((value) => {
//     // If "All Time" is selected, pass empty array
//     const filteredTime = value.includes("All Time") ? [] : value.filter(v => v !== "All Time");
//     setTimeFilter(filteredTime);
//     setCurrentPage(1);
//   }, []);
...
//       dateTo = "";
//     }
//     debouncedFetchQuoteData(value, statusFilter, dateFrom, dateTo, 1);
//   }, [statusFilter, timeFilter, dateRange, debouncedFetchQuoteData]);
...
//       dateTo = "";
//     }
//     fetchQuoteData(searchQuery, [], dateFrom, dateTo, 1);
//   }, [searchQuery, timeFilter, dateRange]);

//   const handleTimeRemove = useCallback(() => {
//     setTimeFilter([]);
//     setCurrentPage(1);
//     fetchQuoteData(searchQuery, statusFilter, "", "", 1);
...
//       dateTo = "";
//     }
//     fetchQuoteData("", statusFilter, dateFrom, dateTo, 1);
//   }, [statusFilter, timeFilter, dateRange]);
...
// // const { storeData, hasFetched } = useSelector((state) => state.store);
...
//           console.log("----aanalyticDta",setAnalyticsDatas);
...
// //     if (!hasFetched) {
// //       console.log("📦 Store data not yet fetched — dispatching API call");
// //       const resultAction = await dispatch(fetchStoreData());
// //       if (fetchStoreData.rejected.match(resultAction)) {
...
// //       }
// //     } else {
// //       console.log("✅ Store data already present:", storeData);
// //     }

// //     await fetchQuoteData(searchQuery, statusFilter, undefined, undefined, currentPage);

// //     try {
// //       const analytics = await APIServ.getAnalyticsDetails();
// //       setAnalyticsData(analytics.result);
// //     } catch (err) {
...
// //   loadData();
// // }, [hasFetched]); // ✅ this only reruns if hasFetched changes
